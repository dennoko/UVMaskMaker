...
namespace Dennoko.UVTools
{
    ...
    internal static class DennokoUIFont
    {
        ...
        // 基本語彙（テンプレート由来）＋ res/lang/*.json と UXML に出る全文字。
        // UI 文言を追加したらここも更新する。
        private const string WarmupJapanese =
            "適用保存解除追加削除設定選択中対象有効無効表示非切替更新確認取消閉開始了はいいえ完了失敗警告情報エラー成功準備実行処理読込書出" +
            "、。あかがきぎくけこさしじすずせただっつづてでとどなにねのびぶへべまみむめもやよらりるれわを" +
            "んァアイェォオカキクグゲコゴサシジスズセタダチッテデトドネバビピフブプベペホポマムメモャヤュ" +
            "ョリルレロワン・一三上下与付以位作使便値停像元先全再分別利制前力動化半単去参反受可合同名向含" +
            "回固在域基場塗境変外太奪子島左常度張形影後従得必応戻手扱抑押抽拡括持指描操数整方既明映時暗最" +
            "未本析果業標止法済点焼照版特独現環生画界番白的目直立細終結緑線編背能膨自致色英要見規角言詳語" +
            "赤転近透通速連避部配重閾防集青静面響頂領高黒（）？";
